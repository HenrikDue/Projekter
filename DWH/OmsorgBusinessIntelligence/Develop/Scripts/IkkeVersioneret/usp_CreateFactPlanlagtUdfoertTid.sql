...
/****** Object:  StoredProcedure [dbo].[usp_CreateFactPlanlagtUdfoertTid]    Script Date: 11/24/2010 09:54:29 ******/
SET ANSI_NULLS ON
GO
...
set @cmd = 'SELECT' +char(13)+  --HUSK
           ...
           '   (MEDHISTORIK.IKRAFTDATO<SPR.RETDATO) AND (MEDHISTORIK.SLUTDATO>SPR.RETDATO)),9999) AS MED_ORG, ' +char(13)+ 
           '  COALESCE((SELECT MEDARBEJDER_STATUSID FROM MEDHISTORIK WHERE MEDHISTORIK.MEDARBEJDERID=SPR.MEDID AND ' +char(13)+
           '   (MEDHISTORIK.IKRAFTDATO<SPR.RETDATO) AND (MEDHISTORIK.SLUTDATO>SPR.RETDATO)),9999) AS MEDARBEJDER_STATUSID, ' +char(13)+ 
           '  COALESCE((SELECT STILLINGSID FROM MEDHISTORIK WHERE MEDHISTORIK.MEDARBEJDERID=SPR.MEDID AND ' +char(13)+
           '   (MEDHISTORIK.IKRAFTDATO<SPR.RETDATO) AND (MEDHISTORIK.SLUTDATO>SPR.RETDATO)),9999) AS STILLINGSID, ' +char(13)+ 
           '  SPR.VEJTID, ' +char(13)+
           ...
           --'  CASE WHEN '+@vejtidibesoeg+'=1 THEN ' +char(13)+  --SPR.RTID er hele beøgets længde        
           --'    (SPR.RTID-SPR.RVEJTID)  ' +char(13)+ --afhængig af system opsætning
           --'  ELSE SPR.RTID ' +char(13)+
           --'  END AS MOBIL_TID, ' +char(13)+
...
set @cmd = 'SELECT ' +char(13)+
           ...
           '  SPRD.YDELSESTID AS PLANLAGT_TID , ' +char(13)+  --planlagte tid 
           '  CASE WHEN SPRD.STATUSID=1 THEN ' +char(13)+ --udført tid 
